...
<div class="form-group has-feedback ">
    ...
    <div class="col-sm-10">
        ...
        @foreach($data -> category as $key => $cat)
                <option name="{!! $cat -> category !!}" value="{!!$cat -> id !!}">{!! $cat -> category !!}</option>
            @endforeach
        </select>
...
<div class="form-group has-feedback ">
    @if (! empty($data -> collection -> image))
        ...
        <div class="col-sm-8">
            ...
            <a href="{{ url('media/products') .'/'. $data -> collection -> image }} " class="current-attachment">
                <img src="{{ url('media/products') .'/'. $data -> collection -> image }} " alt="" width="150">
            </a>
        </div>
